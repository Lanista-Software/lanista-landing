...
<template>
    <MolAppSectionLayout :title="title" :description="description">
        <div v-if="Object.keys(filteredCategories).length" class="w-full">
                    ...
                    <LuiTabPanel v-for="(items, category) in filteredCategories" :key="category" :id="category">
                        <ul>
                            <li v-for="item in items" :key="item.title"
                                class="flex items-center justify-between py-5 border-b border-border-color last:border-b-0">
                                <div class="flex items-center space-x-4">
                                    <!-- Teknoloji logosu -->
                                    <LuiAvatar :src="item.logo" color="secondary" size="xl" rounded="full"/>
                                    <!-- Başlık ve açıklama -->
                                    <div>
                                        <h3 class="text-lg font-medium text-heading-text">{{ item.title }}</h3>
